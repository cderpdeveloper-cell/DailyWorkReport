...
export const ProjectsPage: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await updateProject(editingId, { name, isActive: true });
        toast.success("Project updated");
      } else {
        await createProject({ name, isActive: true });
        ...
        {projects.map((project) => (
          <div key={project.id} className="card flex items-center justify-between p-4">
            <div className="flex items-center gap-4">
              ...
              <span className={`font-medium ${!project.isActive ? 'text-gray-400 line-through' : ''}`}>
                {project.name}
              </span>
            </div>
            <div className="flex gap-2">
              <Button 
                ...
                onClick={() => { setEditingId(project.id); setName(project.name); }}
              >
                <Edit2 size={20} />
              </Button>
              <Button 
